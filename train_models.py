...
class TransformerEncoderModel(nn.Module):
    ...
    def forward(self, input, other_features=None):
...
class TransformerEncoderDecoderModelOrder3(nn.Module):
    ...
    def __init__(self, ntokens=[4, 16, 64], embedding_size=512, hidden_size=[2048, 2048, 2048], hidden_size_fully=None,
                 output_size=1, nhead=8, num_encoder_layers=[6, 6, 6], dropout=0.1, softmax_bool=False,
                 other_size=0, encoding_dim=0, dnabert_dim=0, dnabert_path=None, dnabert_finetune=False):
        ...
        if encoding_dim > 0:
            self.enc_conv1 = nn.Conv1d(8, 32, kernel_size=5, padding=2)
            self.enc_conv2 = nn.Conv1d(32, 64, kernel_size=5, padding=2)
            self.enc_pool = nn.AdaptiveMaxPool1d(1)
            self.enc_fc = nn.Linear(64, encoding_dim)
        ...
        if hidden_size_fully is None:
            ...
            self.fully_connected_layers = nn.Sequential(
                #nn.Linear(self.n*len(self.ntokens)*embedding_size + self.other_size, hidden_size_fully[0]),
                nn.Linear(self.n*len(self.ntokens)*embedding_size + self.other_size + self.encoding_dim + self.dnabert_dim, hidden_size_fully[0]),
                nn.ReLU(),
                nn.Dropout(dropout),
                nn.Linear(hidden_size_fully[0], output_size), )
        elif len(hidden_size_fully) == 2:
    ...
    def forward(self, input, other_features=None, encoding_matrix=None, dnabert_emb=None, dnabert_ids=None, dnabert_mask=None):
        ...
        if getattr(self, 'other_size', 0) > 0:
            combined = torch.cat([combined, other_features], 1)
        # combined = torch.cat((memories[0], memories[1], memories[2], input[3]), 1)
        #output = (self.fully_connected_layers(combined))

        # -- NOVO: ramo CNN da Fase 3 --
        #if self.encoding_dim > 0:
        if getattr(self, 'encoding_dim', 0) > 0:
            e = self.enc_relu(self.enc_conv1(encoding_matrix))   # (batch, 32, 73)
            e = self.enc_relu(self.enc_conv2(e))                 # (batch, 64, 73)
            e = self.enc_pool(e).squeeze(-1)                     # (batch, 64)
            e = self.enc_relu(self.enc_fc(e))                    # (batch, encoding_dim)
            combined = torch.cat([combined, e], 1)
...
def train_transformer_order3(transformer, criterion, optimizer, scheduler, X_train, X_test, y_train, y_test,
    ...
    for epoch in range(epoch_num):
        ...
        for i in range(batch_num):
            ...
            # NOVO: tensor de features escalares (as 17 features normalizadas)
            ...
            optimizer.zero_grad()
            dnabert_ids = torch.tensor(list(xb["Dnabert_ids"]), device=device, dtype=torch.long)
            dnabert_mask = torch.tensor(list(xb["Dnabert_mask"]), device=device, dtype=torch.long)
            outputs, _ = transformer(input, other_features=other_feats, encoding_matrix=enc_matrix,
                                    dnabert_emb=dnabert_emb, dnabert_ids=dnabert_ids, dnabert_mask=dnabert_mask)
            loss = criterion(outputs.squeeze(), torch.tensor(list(yb), device=device, dtype=torch.float32))
            loss.backward()
            # torch.nn.utils.clip_grad_norm_(rnn.parameters(), 0.25)
            optimizer.step()
...
def train_and_test_transformer(X_train, X_test, y_train, y_test, hyperparameters, transformer=None):
    ...
    if transformer is None or not hyperparameters['transfer']:
        ...
        transformer = TransformerEncoderModel(ntoken=4, embedding_size=hyperparameters['embedding_size'],
                                                    ...
                                                    num_encoder_layers=hyperparameters['num_encoder_layers'],
                                                    dropout=hyperparameters['drop_out'], softmax_bool=False,
                                                    other_size=hyperparameters['other_size'],
                                                    dnabert_dim=hyperparameters['dnabert_dim'],           
                                                    dnabert_finetune=hyperparameters.get('dnabert_finetune', False),
                                                    dnabert_path=hyperparameters.get('dnabert_path', None).to(device))
...
def train_and_test_transformer_order3(X_train, X_test, y_train, y_test, hyperparameters, transformer=None):
    ...
    #optimizer = torch.optim.Adam(transformer.parameters(), lr=hyperparameters['lr'],
    #                             weight_decay=hyperparameters['weight_decay'])
    if hyperparameters.get('dnabert_finetune', False):
        optimizer = torch.optim.Adam([
            {'params': [p for n, p in transformer.named_parameters() if 'dnabert' in n],
            'lr': hyperparameters.get('dnabert_lr', 2e-5)},
            {'params': [p for n, p in transformer.named_parameters() if 'dnabert' not in n],
